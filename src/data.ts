...
export function getWeekLabels(mondays: number[]) {
...
export function combineCSVData(trxCsv: string, salesCsv: string, penaltyCsv?: string): { restaurants: RestaurantData[], mondays: number[] } {
  ...
  const restaurants = RESTAURANT_NAMES.map(name => {
  ...
  return { restaurants, mondays: allMondays };
}
...
export const PROCESSED_DATA = combineCSVData(INITIAL_CSV, INITIAL_CSV).restaurants;
